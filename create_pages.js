...
const rawTerms = fs.readFileSync('terms_raw.txt', 'utf8');

const privacyContent = `import { Navbar } from "@/components/navbar"
...
fs.writeFileSync('app/privacy/page.tsx', privacyContent);

const termMatches = rawTerms.match(/(\d+\.\s+[^\n]+)\n([\s\S]*?)(?=\n\d+\.\s+|$)/g) || [];

let termsJsx = '';
termMatches.forEach(m => {
  const nlIdx = m.indexOf('\n');
  const h = m.substring(0, nlIdx).trim();
  const rawBody = m.substring(nlIdx).trim();
  
  const pTags = rawBody.split('\n\n').map(p => {
    if (p.startsWith('•')) {
      ...
      return '<p className="leading-relaxed mb-4">' + safeP + '</p>';
    }
  }).join('\n');
  
  const safeH = h.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;');

  termsJsx += `
            <section>
              <h2 className="text-2xl font-bold text-white mb-4">${safeH}</h2>
              ${pTags}
            </section>
  `;
});

const termsContent = `import { Navbar } from "@/components/navbar"
import { Footer } from "@/components/footer"
import type { Metadata } from "next"
...
fs.writeFileSync('app/terms/page.tsx', termsContent);
