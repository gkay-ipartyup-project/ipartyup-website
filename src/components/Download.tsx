...
function PlatformCard({ platform, index }: PlatformCardProps) {
    ...
    const isBackVersion =
        ...
        platform.download!.version !== platform.latestVersion;
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center text-center h-full">
                ...
                <div className="flex flex-col items-center gap-2 mb-6 md:mb-8 min-h-[48px]">
                    <span
                        className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg border transition-all duration-500 ${
                            !available
                    ...
                    {isBackVersion && platform.latestVersion && (
                        <span className="text-[10px] text-amber-300/50 font-semibold tracking-wide">
                            {formatVersion(platform.latestVersion)} available — {platform.name} build pending
                        </span>
                    )}
                </div>
...
function DownloadButton({ platform }: { platform: Platform }) {
    ...
    if (disabled) {
        return (
            <div
                aria-disabled
                className="mt-auto w-full py-4 bg-white/[0.02] text-white/30 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 border border-white/5 cursor-not-allowed select-none"
            >
                <Lock size={14} />
                <span>{platform.loading ? "Loading" : "Coming Soon"}</span>
            </div>
        );
    }
...
export default function DownloadSection() {
    ...
    useEffect(() => {
        ...
        getLatestDownloads(ctrl.signal)
            .then((d) => setData(d))
        ...
        <section id="download" className="py-20 md:py-32 px-4 md:px-6 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    ...
                    <p className="text-lg md:text-xl lg:text-2xl text-white/40 font-medium tracking-tight">
